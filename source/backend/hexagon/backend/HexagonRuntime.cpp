...
void HexagonRuntime::flushCommand() const {
    ...
    if (recordPath != nullptr && recordPath[0] != '\0' && mCommandGroupCount == 0) {
    ...
    if (recordPath != nullptr && recordPath[0] != '\0' && mCommandGroupCount > 0) {
        ...
        auto allBuffers = snapshotDspMappedBuffers();
        ...
        if (file != nullptr) {
            ...
            if (!mOfflineRecordedCommands.empty()) {
                ...
                if (lastCommand->outputs() != nullptr && lastCommand->outputs()->size() > 0) {
                    const auto* output = lastCommand->outputs()->Get(lastCommand->outputs()->size() - 1);
                    mOfflineOutputFd = static_cast<uint32_t>(output->fd());
                    mOfflineOutputOffset = static_cast<uint32_t>(output->offset());
                    for (const auto& record : mPendingHexagonOutputs) {
            ...
            if (outputBytesOverride != nullptr && outputBytesOverride[0] != '\0') {
                const unsigned long outputBytes = strtoul(outputBytesOverride, nullptr, 10);
                if (outputBytes > 0 && outputBytes <= UINT32_MAX) {
                    mOfflineOutputSize = static_cast<uint32_t>(outputBytes);
                }
            }
            ...
            for (const auto& buffer : mOfflineRecordedBuffers) {
                ...
                bufferDescs.push_back({buffer.fd, static_cast<uint32_t>(buffer.data.size()), buffer.alignment,
                                       buffer.flags, static_cast<uint32_t>(chunks.size())});
            ...
            ok = ok && fwrite(bufferDescs.data(), sizeof(OfflineRpcBufferDesc), bufferDescs.size(), file) ==
                           bufferDescs.size();
            ok = ok && fwrite(commandDescs.data(), sizeof(OfflineRpcCommandDesc), commandDescs.size(), file) ==
                           commandDescs.size();
            ...
            for (size_t bufferIndex = 0; ok && bufferIndex < mOfflineRecordedBuffers.size(); ++bufferIndex) {
                ...
                ok = fwrite(chunks.data(), sizeof(OfflineRpcChunkDesc), chunks.size(), file) == chunks.size();
            ...
            MNN_PRINT("[Hexagon][OfflineRPC] recorded %zu commands, %zu buffers, %zu stored bytes to %s, ok=%d\n",
                      mOfflineRecordedCommands.size(), mOfflineRecordedBuffers.size(), storedBytes, recordPath,
                      ok ? 1 : 0);
        }
        for (const auto& chunk : mQueuedCommandChunks)
            mCommandAlloc->free(chunk);
