...
static bool topk_parallel_row(const __fp16* row, int32_t rowSize, int32_t k, float* outputValues,
                              int32_t* outputIndices, int* profile) {
  ...
  const unsigned long long scanStart = HAP_perf_get_time_us();
  int32_t submitted = 0;
  ...
  worker_pool_synctoken_wait(&state.sync);
  const unsigned long long scanUs = HAP_perf_get_time_us() - scanStart;
  float values[kMaxTopK];
  int32_t indices[kMaxTopK];
  int32_t size = 0;
  const unsigned long long mergeStart = HAP_perf_get_time_us();
  ...
  const unsigned long long mergeUs = HAP_perf_get_time_us() - mergeStart;
  if (profile) {
    profile[DSP_OP_TOPKV2_SCAN_PROFILE] += (int)scanUs;
    profile[DSP_OP_TOPKV2_MERGE_PROFILE] += (int)mergeUs;
  }
  free(candidates);
  free(candidateIndices);
  return true;
...
AEEResult htp_ops_topkv2_k1_fp16(uint8_t* values, uint8_t* indices, uint8_t* input, int32_t rowSize, int32_t rows,
  ...
  unsigned long long scanUs = 0;
  unsigned long long outputUs = 0;
  for (int r = 0; r < rows; ++r) {
    const __fp16* row = src + (size_t)r * rowSize;
    const unsigned long long scanStart = profile ? HAP_perf_get_time_us() : 0;
    ...
    if (profile) {
      scanUs += HAP_perf_get_time_us() - scanStart;
    }
    const unsigned long long outputStart = profile ? HAP_perf_get_time_us() : 0;
    ((uint16_t*)valueOut)[r] = bestBits;
    indexOut[r] = bestIndex;
    if (profile) {
      outputUs += HAP_perf_get_time_us() - outputStart;
    }
  }
  if (profile) {
    profile[DSP_OP_TOPKV2_SCAN_PROFILE] += (int)scanUs;
    profile[DSP_OP_TOPKV2_OUTPUT_PROFILE] += (int)outputUs;
  }
  return 0;
}
...
AEEResult htp_ops_topkv2_fp16(uint8_t* values, uint8_t* indices, uint8_t* input,
                              int32_t rowSize, int32_t rows, int32_t k, int32_t outputBytes, int* profile) {
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    if (topk_parallel_row(row, rowSize, k, parallelValues, parallelIndices, profile)) {
      const unsigned long long outputStart = profile ? HAP_perf_get_time_us() : 0;
      for (int32_t j = 0; j < k; ++j) {
        if (outputBytes == 4) {
