...
int runVisionBlock(const char *modelPath, MNNForwardType backend, const char *outputPath) {
  std::shared_ptr<Interpreter> interpreter(Interpreter::createFromFile(modelPath), Interpreter::destroy);
  if (!interpreter) {
    return 2;
  }
  ScheduleConfig config;
  BackendConfig  backendConfig;
  backendConfig.precision = backend == MNN_FORWARD_CPU ? BackendConfig::Precision_Normal : BackendConfig::Precision_Low;
  config.type             = backend;
  config.backendConfig    = &backendConfig;
  Session *session        = interpreter->createSession(config);
  if (!session) {
  ...
  for (int row = 0; row < kRows; ++row) {
    for (int channel = 0; channel < 1024; ++channel) {
      hostInput.host<float>()[row * 1024 + channel] = inputValue(row, channel);
    }
  }
  input->copyFromHostTensor(&hostInput);

  const auto sessionInputs = interpreter->getSessionInputAll(session);
  const auto positionIter  = sessionInputs.find("position_ids");
  if (positionIter != sessionInputs.end()) {
    Tensor *positionIds = positionIter->second;
    Tensor  hostPositionIds(positionIds, positionIds->getDimensionType());
    for (int row = 0; row < kRows; ++row) {
      hostPositionIds.host<int32_t>()[row]         = row / 2;
      hostPositionIds.host<int32_t>()[kRows + row] = row % 2;
    }
    positionIds->copyFromHostTensor(&hostPositionIds);
  }

  const auto maskIter = sessionInputs.find("attention_mask");
  if (maskIter != sessionInputs.end()) {
    Tensor *attentionMask = maskIter->second;
    Tensor  hostAttentionMask(attentionMask, attentionMask->getDimensionType());
    memset(hostAttentionMask.host<float>(), 0, hostAttentionMask.size());
    attentionMask->copyFromHostTensor(&hostAttentionMask);
  }
  ...
  printf("mnn_vision_block_record: backend=%d model=%s code=%d\n", backend, modelPath, code);
  return 0;
}
