...
ErrorCode HexagonLinearAttention::onBuildCmd(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
    ...
    std::vector<int> params = {
        mBatch,           mConvDim,          mSequence,         mNumKHeads,           mNumVHeads,
    ...
    std::vector<std::pair<int, int>> inputFds = {HexagonBackend::getDevicePtr(inputs[0]),
                                                 HexagonBackend::getDevicePtr(inputs[1]),
                                                 HexagonBackend::getDevicePtr(inputs[2]),
                                                 HexagonBackend::getDevicePtr(inputs[3]),
                                                 HexagonBackend::getDevicePtr(mState->conv.get()),
                                                 HexagonBackend::getDevicePtr(mState->recurrent.get()),
    ...
    std::vector<Tensor*> commandInputs = {inputs[0],
                                          inputs[1],
                                          inputs[2],
                                          inputs[3],
                                          mState->conv.get(),
                                          mState->recurrent.get(),
                                          mState->packedConvWeight.get()};
