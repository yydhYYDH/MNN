...
std::vector<int> Omni::qwen2VisionProcess(VARP image) {
    ...
    const int interpolation = isQwen3VL ? MNN::CV::INTER_PILLOW_BICUBIC : MNN::CV::INTER_LINEAR;
    image = MNN::CV::resize(image, {mVisionWidth, mVisionHeight}, 0, 0, interpolation, MNN::CV::COLOR_BGR2RGB,
                            mVisionMean, mVisionNorm);
    image = Express::_Unsqueeze(image, {0});
